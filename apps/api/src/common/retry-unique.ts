 ...
 *
 * Used for sequence-style human IDs (studentId `YYYY####`, staffId `STF-####`)
 * derived from a row count. Under concurrent inserts two requests can read the
 * same count and generate the same ID; the unique index makes the loser throw
 * P2002. Recomputing the sequence inside a fresh attempt resolves the race.
 *
