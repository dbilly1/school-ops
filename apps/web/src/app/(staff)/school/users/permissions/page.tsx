...
// Feature + sub-feature structure (mirrors the plan)
const FEATURES = [
...
function FeatureSection({ feature, overrides, defaults, role, onChange, disabled }: {
  ...
  function getDefault(featureKey: string, subFeatureKey: string | null, action: Action): boolean {
    return defaults.find(d => d.featureKey === featureKey && d.subFeatureKey === subFeatureKey && d.action === action)?.allowed ?? false;
  }
  ...
  // Rows to render: top-level feature + each sub-feature
  const rows = [
      ...
      {rows.map((row, i) => {
        ...
        return (
          ...
          >
            {/* Label */}
            <div
              className={`flex-1 px-4 py-2.5 text-sm flex items-center gap-2 ${row.isParent ? 'font-semibold text-slate-700' : 'text-slate-600 pl-8'}`}
            >
...
export default function RolePermissionsPage() {
  ...
  // Owner-only toggle: can Admin manage permissions?
  ...
  // Build override map when data loads
  useEffect(() => {
  ...
  async function saveAll() {
    ...
    try {
      ...
      pendingChanges.forEach((state, key) => {
        ...
        if (state === 'default') {
          // Default = remove override (not directly supported by API — set to role default value)
          // We'll just skip — ideally a DELETE endpoint exists
          return;
        }
        promises.push(
  ...
  return (
    <div>
      {/* Back */}
      <button onClick={() => router.push('/school/users')}
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Override default permissions per role. Click any cell to toggle. Coloured = overridden.</p>
        </div>
        {hasPending && (
          <button
            onClick={saveAll}
      ...
      {isOwner && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5 py-4 mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">When off, only you (the School Owner) can edit role and user permission overrides.</p>
          </div>
          <button
            type="button"
            onClick={async () => {
              ...
              try {
                await staffApi.patch('/school/settings/admin-permission-toggle', { allowed: !adminCanManage });
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="flex items-center border-b border-slate-100 bg-slate-50">
          <div className="flex-1 px-4 py-2.5 text-xs font-semibold text-slate-400 uppercase tracking-wide">Feature / Sub-feature</div>
          {ACTIONS.map(a => (
