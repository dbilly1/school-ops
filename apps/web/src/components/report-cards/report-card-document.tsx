...
export function ReportCardDocument({
  ...
  const scaleOn =
    (cfg.showAssessmentScale ?? false) &&
    !!data.assessmentScale?.levels?.length &&
    !!data.assessmentScale?.skills?.length;
  const metricsOn =
    (cfg.showMetricsTable ?? false) && !!data.gradingBands?.length;
...
function StandardLayout({
  ...
  return (
    <div
      className="bg-white text-slate-900 mx-auto w-full max-w-[820px] p-8 sm:p-10"
      style={{ fontSize: 13 }}
    >
      <div className="text-center">
        ...
        <h2 className="mt-2 text-base font-bold tracking-wide">
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-8 gap-y-1 text-sm">
        <p>
          <span className="font-semibold">Name:</span> {data.student.firstName}{" "}
        ...
        {data.className && (
          <p>
            <span className="font-semibold">Class:</span> {data.className}
          </p>
        )}
        {fmtDate(data.vacationDate) && (
          <p>
            <span className="font-semibold">Vacation:</span>{" "}
            {fmtDate(data.vacationDate)}
          </p>
        )}
        {fmtDate(data.nextTermReopens) && (
          <p>
      ...
      {(cfg.showAttendanceSummary ?? true) && (
        <Attendance data={data} accent={accent} />
      )}
      {scaleOn && (
        <div className="mt-6">
          <AssessmentScaleTable data={data} accent={accent} />
        </div>
      )}
      {scaleOn && (
        <div className="mt-6">
          <HolisticTable data={data} accent={accent} />
        </div>
      )}
      {metricsOn && (
        <div className="mt-6">
          <MetricsTable data={data} accent={accent} />
        </div>
      )}
      {(cfg.showBehaviourScores ?? false) && (
        <Conduct data={data} accent={accent} />
      )}
      {(cfg.showTeacherComments ?? true) && (
        <RemarkBox
          accent={accent}
          label="Class Teacher's Comments"
          text={data.conduct?.teacherRemarks}
        />
      )}
      {(cfg.showPrincipalComments ?? true) && (
        <RemarkBox
          accent={accent}
          label="Head Teacher's Comments"
          text={data.conduct?.headTeacherRemarks}
        />
      )}
      {cfg.footerText && (
...
function HolisticLayout({
  ...
  return (
    <div
      className="bg-white text-slate-900 mx-auto w-full max-w-[820px]"
      style={{ fontSize: 12.5 }}
    >
      {/* Brand header band */}
      <div
        ...
        style={{ backgroundColor: accent }}
      >
        <div className="w-6 shrink-0" style={{ backgroundColor: "#f59e0b" }} />
        <div className="flex flex-1 items-center gap-4 pr-6 ">
          {school.logoUrl && (
            <span className="self-stretch flex items-center justify-center bg-white px-3">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={school.logoUrl}
                alt=""
          ...
          <div className="flex flex-1 items-center gap-4 py-3">
            <div className="flex-1">
              <p className="text-lg font-extrabold uppercase tracking-wide leading-tight">
                {school.name}
              </p>
              {school.address && (
                <p className="text-[11px] opacity-90">{school.address}</p>
              )}
            </div>
      ...
      <div className="p-6 sm:p-8">
        ...
        <table className="w-full border-collapse text-sm">
          <tbody>
            <tr>
              <Cell>
                <b>Name:</b> {data.student.firstName} {data.student.lastName}
              </Cell>
              <Cell>
            ...
            <tr>
              <Cell>
                <b>Academic Year/Term:</b> {data.term.academicYear.name} –{" "}
                {data.term.name}
              </Cell>
              <Cell>
                <b>Vacation:</b> {fmtDate(data.vacationDate) || "—"}
              </Cell>
            </tr>
            <tr>
              <Cell>
                <b>Class Teacher:</b> {data.classTeacherName ?? "—"}
              </Cell>
              <Cell>
                <b>Next Term Begins:</b> {fmtDate(data.nextTermReopens) || "—"}
              </Cell>
            </tr>
          </tbody>
        </table>
        ...
        <div className="mt-5">
          <SubjectsTable data={data} accent={accent} />
        </div>
        <Summary data={data} cfg={cfg} />
      ...
      {/* Footer motto band */}
      <div
        className="text-center text-white text-sm font-semibold tracking-[0.3em] uppercase py-2.5"
        style={{ backgroundColor: accent }}
      >
        {cfg.footerText || ""}
      </div>
...
function SectionTitle({
  ...
  return (
    <tr>
      <th
        colSpan={span}
        className={`${BORDER} px-3 py-1.5 text-center font-bold uppercase tracking-wide`}
        style={{ color: accent }}
      >
        {children}
      </th>
...
function SubjectsTable({
  ...
  return (
    <table
      className={`${filledHeader ? "mt-5" : ""} w-full border-collapse text-sm`}
    >
      <thead>
      ...
      <tbody>
        {data.subjects.map((s) => (
          <tr key={s.subjectId}>
            ...
            <td className={`${BORDER} px-3 py-1.5 text-center`}>
              {s.sbaPercent != null ? s.sbaScore : "—"}
            </td>
            <td className={`${BORDER} px-3 py-1.5 text-center`}>
              {s.examPercent != null ? s.examScore : "—"}
            </td>
            ...
            {showGrade && (
              <td className={`${BORDER} px-3 py-1.5`}>{s.remark ?? ""}</td>
            )}
          </tr>
        ))}
        {data.subjects.length === 0 && (
          <tr>
            <td
              colSpan={6}
              className={`${BORDER} px-3 py-6 text-center text-slate-400`}
            >
              No scored subjects for this term yet.
            </td>
          </tr>
        )}
      </tbody>
...
function Summary({
  ...
  data: ReportCardData;
  cfg: NonNullable<ReportCardData["config"]>;
}) {
  ...
  return (
    <div className="mt-4">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Present: {data.attendance.presentDays} / {data.attendance.totalDays} ·
        Absent: {data.attendance.absentDays} · Rate: {data.attendance.rate}%
      </p>
    </div>
  );
}
...
function AssessmentScaleTable({
  ...
  return (
    <table className="w-full border-collapse text-sm">
      <thead>
        <SectionTitle span={3} accent={accent}>
          Assessment Scale
        </SectionTitle>
      </thead>
      <tbody>
        {levels.map((l) => (
          <tr key={l.id}>
            <td className={`${BORDER} px-3 py-1.5 text-center font-bold w-12`}>
              {l.code}
            </td>
            <td className={`${BORDER} px-3 py-1.5 font-medium w-48`}>
              {l.label}
            </td>
            <td className={`${BORDER} px-3 py-1.5 text-slate-600`}>
              {l.description ?? ""}
            </td>
          </tr>
        ))}
    ...
    <table className="w-full border-collapse text-sm">
      <thead>
        <tr className="text-slate-700">
          <th className={`${BORDER} px-3 py-1.5 text-left font-bold`}>
            {group}
          </th>
          {scale.levels.map((l) => (
            <th
              key={l.id}
              className={`${BORDER} px-2 py-1.5 text-center font-semibold w-12`}
            >
              {l.code}
            </th>
          ))}
  ...
  return (
    <table className="w-full border-collapse text-sm">
      ...
      <tbody>
        {bands.map((b, i) => (
          <tr key={i}>
            <td className={`${BORDER} px-3 py-1.5 text-center font-bold w-24`}>
              {b.label}
            </td>
            <td className={`${BORDER} px-3 py-1.5 w-48`}>
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mt-1 space-y-0.5 text-sm text-slate-600">
        {c.attitudes && (
          <p>
            <span className="font-semibold">Attitudes:</span> {c.attitudes}
          </p>
        )}
        {c.interests && (
          <p>
            <span className="font-semibold">Interests:</span> {c.interests}
          </p>
        )}
        {c.conduct && (
          <p>
            <span className="font-semibold">Conduct:</span> {c.conduct}
          </p>
        )}
      </div>
    </div>
